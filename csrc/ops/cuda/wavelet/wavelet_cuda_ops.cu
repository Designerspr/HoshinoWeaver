...
void launch_wavelet_dec_rec_cuda_core(const double* image_host, double* out_host, const int height,
    ...
    throw_if_cuda_failed(cudaMemcpy(current.get(), image_host, current_size * sizeof(double),
                                    cudaMemcpyHostToDevice),
                         "wavelet_dec_rec_cuda_core cudaMemcpy input");

    DeviceImage result = wavelet_dec_rec_device(std::move(current), height, width, level, threads);
    throw_if_cuda_failed(cudaMemcpy(out_host, result.data.get(), result.size * sizeof(double),
