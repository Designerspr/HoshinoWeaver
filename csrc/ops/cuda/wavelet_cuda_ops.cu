...
int next_power_of_two_int(const int value) {
    ...
    if (value > (std::numeric_limits<int>::max() / 2 + 1)) {
        throw std::runtime_error(
            "star_detect_connected_components: hash table is too large");
    }
...
__global__ void resize_linear_mask_kernel(const double* input,
                                          const uint8_t* mask,
                                          double* output,
                                          const int in_h,
                                          const int in_w,
    ...
    const int y = idx / out_w;
    const int x = idx - y * out_w;
    const double src_y = (static_cast<double>(y) + 0.5) *
                             static_cast<double>(in_h) / static_cast<double>(out_h) -
                         0.5;
    const double src_x = (static_cast<double>(x) + 0.5) *
                             static_cast<double>(in_w) / static_cast<double>(out_w) -
                         0.5;
...
__global__ void resize_linear_kernel(const double* input,
                                     double* output,
                                     const int in_h,
                                     const int in_w,
                                     const int out_h,
...
__global__ void gaussian_rows_kernel(const double* input,
                                     const double* kernel,
                                     double* output,
                                     const int height,
                                     const int width,
...
__global__ void gaussian_cols_kernel(const double* input,
                                     const double* kernel,
                                     double* output,
                                     const int height,
                                     const int width,
...
__global__ void normalize_kernel(const double* input,
                                 double* output,
                                 const double mean,
                                 const double range,
                                 const int total) {
...
__global__ void threshold_small_dark_mask_kernel(const double* image,
                                                uint8_t* mask,
                                                const int total,
...
__global__ void dilate_mask_kernel(const uint8_t* input,
...
__global__ void resize_allowed_mask_kernel(const uint8_t* dark_mask,
                                           const uint8_t* external_mask,
                                           uint8_t* output,
                                           int* allowed_count,
                                           const int in_h,
                                           const int in_w,
...
__global__ void compact_masked_values_kernel(const double* image,
                                             const uint8_t* mask,
                                             double* values,
                                             int* count,
                                             const int total) {
...
__device__ inline uint8_t threshold_pixel(const double value,
                                          const uint8_t mask,
                                          const double threshold) {
    return (mask != 0 && value > threshold) ? 255 : 0;
}

__global__ void erode_threshold_kernel(const double* image,
                                       const uint8_t* mask,
                                       uint8_t* eroded,
                                       const int height,
                                       const int width,
    ...
    for (int dy = -1; dy <= 1; ++dy) {
        ...
        for (int dx = -1; dx <= 1; ++dx) {
            ...
            const uint8_t candidate =
                threshold_pixel(image[offset], mask[offset], threshold);
...
__global__ void dilate_kernel(const uint8_t* eroded,
                              uint8_t* out,
                              const int height,
                              const int width) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void init_component_labels_kernel(const uint8_t* bw,
                                             int* labels,
                                             const int total) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= total) {
...
__global__ void propagate_component_labels_kernel(const int* in_labels,
                                                  int* out_labels,
                                                  int* changed,
                                                  const int height,
                                                  const int width) {
...
__global__ void compact_foreground_indices_kernel(const uint8_t* bw,
                                                  int* indices,
                                                  int* count,
                                                  const int total) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__device__ int find_component_slot(int* keys,
                                   int* overflow,
                                   const int capacity,
                                   const int root_label) {
    const int mask = capacity - 1;
    unsigned int pos = component_hash(static_cast<unsigned int>(root_label)) &
...
__global__ void count_component_outputs_kernel(const int* keys,
                                               const int* counts,
                                               int* out_count,
                                               const int hash_capacity) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void fill_component_outputs_kernel(const int* keys,
                                              const int* counts,
                                              const double* sum_x,
                                              const double* sum_y,
                                              const double* sum_x2,
    ...
    const double det_term =
        sqrt(fmax(0.0, (cov_xx - cov_yy) * (cov_xx - cov_yy) +
                         4.0 * cov_xy * cov_xy));
...
double percentile_sorted(const double* values,
                         const int count,
...
void launch_star_detect_full_connected_components(
    ...
    gaussian_kernel.allocate(
        static_cast<size_t>(gaussian_ksize),
        "star_detect_full_connected_components cudaMalloc gaussian kernel");
    blur_rows.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc blur rows");
    blur.allocate(plane_size, "star_detect_full_connected_components cudaMalloc blur");
    normalized.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc normalized");
    ...
    small_blur.allocate(
        small_size, "star_detect_full_connected_components cudaMalloc small blur");
    img_rec.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc img_rec");
    values.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc values");
    dilate_structuring.allocate(
        static_cast<size_t>(dilate_height) * static_cast<size_t>(dilate_width),
        "star_detect_full_connected_components cudaMalloc dilate kernel");
    small_dark_mask.allocate(
        small_size, "star_detect_full_connected_components cudaMalloc small dark mask");
    ...
    if (external_mask_host != nullptr) {
        external_mask.allocate(
            plane_size, "star_detect_full_connected_components cudaMalloc external mask");
    }

    throw_if_cuda_failed(cudaMemcpy(image.get(),
                                    image_host,
                                    plane_size * sizeof(double),
                                    cudaMemcpyHostToDevice),
                         "star_detect_full_connected_components cudaMemcpy image");
    throw_if_cuda_failed(cudaMemcpy(gaussian_kernel.get(),
                                    gaussian_kernel_host,
                                    static_cast<size_t>(gaussian_ksize) * sizeof(double),
                                    cudaMemcpyHostToDevice),
                         "star_detect_full_connected_components cudaMemcpy gaussian kernel");
    throw_if_cuda_failed(cudaMemcpy(dilate_structuring.get(),
                                    dilate_kernel_host,
    ...
    if (external_mask_host != nullptr) {
        throw_if_cuda_failed(cudaMemcpy(external_mask.get(),
                                        external_mask_host,
                                        plane_size * sizeof(uint8_t),
                                        cudaMemcpyHostToDevice),
    ...
    gaussian_rows_kernel<<<blocks, threads>>>(
        image.get(),
        gaussian_kernel.get(),
        blur_rows.get(),
        height,
    ...
    gaussian_cols_kernel<<<blocks, threads>>>(
        blur_rows.get(),
        gaussian_kernel.get(),
        blur.get(),
        height,
    ...
    const double blur_sum =
        thrust::reduce(thrust::device, blur_ptr, blur_ptr + total, 0.0);
    const auto minmax = thrust::minmax_element(
        thrust::device, blur_ptr, blur_ptr + total);
    double blur_min = 0.0;
    double blur_max = 0.0;
    throw_if_cuda_failed(cudaMemcpy(&blur_min,
                                    thrust::raw_pointer_cast(minmax.first),
                                    sizeof(double),
                                    cudaMemcpyDeviceToHost),
                         "star_detect_full_connected_components cudaMemcpy blur min");
    throw_if_cuda_failed(cudaMemcpy(&blur_max,
    ...
    normalize_kernel<<<blocks, threads>>>(
        blur.get(), normalized.get(), blur_mean, blur_range, total);
    throw_if_cuda_failed(cudaGetLastError(),
                         "star_detect_full_connected_components normalize launch");
    blur.reset();

    resize_linear_kernel<<<small_blocks, threads>>>(
        image.get(),
        small_mask_values.get(),
        height,
        width,
        small_height,
        small_width);
    ...
    throw_if_cuda_failed(cudaMemcpy(small_mask_sorted.get(),
                                    small_mask_values.get(),
                                    small_size * sizeof(double),
    ...
    dilate_mask_kernel<<<small_blocks, threads>>>(
        small_dark_mask.get(),
        dilate_structuring.get(),
        small_dilated_mask.get(),
        small_height,
        small_width,
        dilate_height,
        dilate_width);
    ...
    resize_allowed_mask_kernel<<<blocks, threads>>>(
        small_dilated_mask.get(),
        external_mask_host == nullptr ? nullptr : external_mask.get(),
        mask.get(),
        count.get(),
        small_height,
        small_width,
    ...
    int allowed_count = 0;
    throw_if_cuda_failed(cudaMemcpy(&allowed_count,
                                    count.get(),
                                    sizeof(int),
                                    cudaMemcpyDeviceToHost),
                         "star_detect_full_connected_components cudaMemcpy mask count");
    if (static_cast<long long>(allowed_count) * 100LL <
        static_cast<long long>(total) * 50LL) {
    ...
    resize_linear_mask_kernel<<<blocks, threads>>>(
        rec_small.data.get(),
        mask.get(),
        img_rec.get(),
        rec_small.h,
    ...
    if (masked_count <= 0) {
        throw std::runtime_error(
            "star_detect_full_connected_components: mask selects no pixels");
    }
    ...
    erode_threshold_kernel<<<blocks, threads>>>(
        img_rec.get(), mask.get(), eroded.get(), height, width, threshold);
    throw_if_cuda_failed(cudaGetLastError(),
                         "star_detect_full_connected_components erode launch");
    dilate_kernel<<<blocks, threads>>>(eroded.get(), bw.get(), height, width);
    throw_if_cuda_failed(cudaGetLastError(),
                         "star_detect_full_connected_components dilate launch");
    eroded.reset();
    mask.reset();

    foreground_indices.allocate(
        plane_size,
        "star_detect_full_connected_components cudaMalloc foreground indices");
    throw_if_cuda_failed(cudaMemset(count.get(), 0, sizeof(int)),
                         "star_detect_full_connected_components cudaMemset foreground count");
    compact_foreground_indices_kernel<<<blocks, threads>>>(
    ...
    throw_if_cuda_failed(cudaMemcpy(&foreground_count,
                                    count.get(),
                                    sizeof(int),
                                    cudaMemcpyDeviceToHost),
    ...
    labels_a.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc labels a");
    labels_b.allocate(
        plane_size, "star_detect_full_connected_components cudaMalloc labels b");
    changed.allocate(1, "star_detect_full_connected_components cudaMalloc changed");
    init_component_labels_kernel<<<blocks, threads>>>(
        bw.get(), labels_a.get(), total);
    throw_if_cuda_failed(cudaGetLastError(),
    ...
    if (!converged) {
        throw std::runtime_error(
            "star_detect_full_connected_components: GPU CC did not converge");
    }
    ...
    keys.allocate(hash_capacity,
                  "star_detect_full_connected_components cudaMalloc hash keys");
    counts.allocate(hash_capacity,
                    "star_detect_full_connected_components cudaMalloc hash counts");
    overflow.allocate(1, "star_detect_full_connected_components cudaMalloc overflow");
    sum_x.allocate(hash_capacity,
                   "star_detect_full_connected_components cudaMalloc sum_x");
    sum_y.allocate(hash_capacity,
                   "star_detect_full_connected_components cudaMalloc sum_y");
    sum_x2.allocate(hash_capacity,
                    "star_detect_full_connected_components cudaMalloc sum_x2");
    sum_y2.allocate(hash_capacity,
                    "star_detect_full_connected_components cudaMalloc sum_y2");
    sum_xy.allocate(hash_capacity,
                    "star_detect_full_connected_components cudaMalloc sum_xy");
    sum_intensity.allocate(
        hash_capacity,
    ...
    throw_if_cuda_failed(
        cudaMemset(sum_intensity.get(), 0, hash_capacity * sizeof(double)),
    ...
    accumulate_component_stats_kernel<<<foreground_blocks, threads>>>(
        ...
        hash_capacity);
    throw_if_cuda_failed(cudaGetLastError(),
                         "star_detect_full_connected_components stats launch");
    ...
    int overflow_flag = 0;
    throw_if_cuda_failed(cudaMemcpy(&overflow_flag,
                                    overflow.get(),
                                    sizeof(int),
                                    cudaMemcpyDeviceToHost),
    ...
    out_positions.allocate(
        static_cast<size_t>(output_count) * 2,
