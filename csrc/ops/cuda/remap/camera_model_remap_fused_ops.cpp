...
using launch_fn_t = void (*)(const T* image_host, T* out_host, int src_height, int src_width,
                             int channels, int out_height, int out_width, float fx_src,
                             float fy_src, float cx_src, float cy_src, float fx_dst, float fy_dst,
                             float cx_dst, float cy_dst, const float* rotation_dst_to_src,
                             bool src_has_dist, const float* src_dist_coeffs, bool dst_has_dist,
                             const float* dst_dist_coeffs);
...
void validate_scalar_finite(const float value, const char* op_name, const char* name,
...
py::array camera_model_remap_fused_dispatch(
    ...
    if (py::isinstance<py::array_t<unsigned char>>(image)) {
        return camera_model_remap_fused_impl<unsigned char>(
            ...
            dst_dist_coeffs, launch_camera_model_remap_fused_u8);
    }
    if (py::isinstance<py::array_t<unsigned short>>(image)) {
        return camera_model_remap_fused_impl<unsigned short>(
            ...
            dst_dist_coeffs, launch_camera_model_remap_fused_u16);
    }
...
void bind_camera_model_remap_fused_ops(py::module_& m) {
    m.def(
        "camera_model_remap", &camera_model_remap_fused_dispatch, py::arg("image"),
