...
template <typename T>
using process_launch_fn_t = void (*)(const T* image_host,
                                     T* out_host,
                                     int height,
                                     int width,
                                     int channels,
...
void validate_process_params(const int shrink_ksize,
    ...
    if (shrink_times <= 0) {
        throw std::invalid_argument(
            "star_shrink_dog_process_cuda: shrink_times must be positive");
    }
    if (!(shrink_ratio > 0.0f && shrink_ratio <= 1.0f)) {
        throw std::invalid_argument(
            "star_shrink_dog_process_cuda: shrink_ratio must be in (0, 1]");
    }
...
py::array star_shrink_dog_process_cuda_dispatch(const py::array& image,
                                                const float sigma_small,
                                                const float sigma_large,
                                                const float threshold_ratio,
                                                const int open_ksize,
...
void bind_star_mask_cuda_ops(py::module_& m) {
    ...
    m.def("star_shrink_dog_process_cuda",
          &star_shrink_dog_process_cuda_dispatch,
          py::arg("image"),
          py::arg("sigma_small"),
          py::arg("sigma_large"),
          py::arg("threshold_ratio"),
