...
void launch_star_shrink_process_cuda_u8(
    const uint8_t* image_host,
...
void launch_star_shrink_process_cuda_u16(
    const uint16_t* image_host,
    const uint8_t* mask_host,
    uint16_t* out_host,
    int height,
    int width,
...
void validate_common(const py::array& image,
    ...
    if (mask.ndim() != 2 || mask.shape(0) != image.shape(0) ||
        ...
        throw std::invalid_argument(
            "star_shrink_process_cuda: star_mask must have shape (H, W)");
    }
    ...
    if (shrink_times <= 0) {
        throw std::invalid_argument(
            "star_shrink_process_cuda: shrink_times must be positive");
    }
    if (!(shrink_ratio > 0.0f && shrink_ratio <= 1.0f)) {
        throw std::invalid_argument(
            "star_shrink_process_cuda: shrink_ratio must be in (0, 1]");
    }
...
py::array_t<T> star_shrink_process_cuda_impl(
    ...
    validate_common(
        image, mask, shrink_ksize, shrink_times, shrink_ratio, deringing_ksize);
...
py::array star_shrink_process_cuda_dispatch(const py::array& image,
...
void bind_star_shrink_process_cuda_ops(py::module_& m) {
    m.def("star_shrink_process_cuda",
          &star_shrink_process_cuda_dispatch,
          py::arg("image"),
          py::arg("star_mask"),
          py::arg("shrink_ksize"),
          py::arg("shrink_shape"),
