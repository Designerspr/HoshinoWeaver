...
void validate_shape(const py::array& stack,
                    const py::array_t<double>& ref_mean,
                    const py::array_t<double>& ref_std,
    ...
    if (ref_mean.ndim() != 1 || ref_std.ndim() != 1 ||
        ref_mean.shape(0) != stack.shape(1) ||
...
py::array_t<double, py::array::c_style | py::array::forcecast> parse_weights(
    const py::object& weights_obj,
...
py::tuple huber_weighted_chunk_cuda_impl(
...

    {
        ...
        launcher(
            ...
            ref_std.data(),
            weights_ptr,
            weighted_sum.mutable_data(),
            weight_total.mutable_data(),
            n_frames,
            plane_size,
            huber_c);
    }
...
py::tuple huber_weighted_chunk_cuda_dispatch(
    ...
    if (py::isinstance<py::array_t<uint8_t>>(stack)) {
        ...
        return huber_weighted_chunk_cuda_impl(
            stack_arr,
            ref_mean,
            ref_std,
            huber_c,
...
void bind_huber_weighted_chunk_cuda_ops(py::module_& m) {
    m.def("huber_weighted_chunk_cuda",
          &huber_weighted_chunk_cuda_dispatch,
          py::arg("stack"),
          py::arg("ref_mean"),
          py::arg("ref_std"),
          py::arg("huber_c"),
          py::arg("weights") = py::none(),
          "CUDA host-in/out Huber weighted chunk accumulation.");
}
