...
void launch_sigma_clip_fused_chunk_cuda_u8(const uint8_t* stack_host, const uint8_t* mask_host,
                                           ...
                                           double* out_n_host, int64_t n_frames, int64_t plane_size,
                                           double rej_high, double rej_low, int max_iter,
                                           bool skip_zero_rgb, int64_t channels);

void launch_sigma_clip_fused_chunk_cuda_u16(const uint16_t* stack_host, const uint8_t* mask_host,
                                            double* out_sum_host, double* out_sq_host,
                                            double* out_n_host, int64_t n_frames,
                                            int64_t plane_size, double rej_high, double rej_low,
                                            int max_iter, bool skip_zero_rgb, int64_t channels);

namespace {

void validate_shape(const py::array& stack, ssize_t channels, const bool skip_zero_rgb) {
    if (stack.ndim() != 2) {
    ...
    if (skip_zero_rgb && channels >= 3 && stack.shape(1) % channels != 0) {
        throw std::invalid_argument("sigma_clip_fused_chunk_cuda: plane_size must be divisible by "
                                    "channels when skip_zero_rgb is true");
    }
}

py::array_t<uint8_t, py::array::c_style | py::array::forcecast>
parse_mask(const py::object& mask_obj, const ssize_t n_frames, const ssize_t plane_size,
    ...
    *mask_ptr = nullptr;
    ...
    if (mask_arr.ndim() != 2 || mask_arr.shape(0) != n_frames || mask_arr.shape(1) != plane_size) {
        throw std::invalid_argument(
...
py::tuple sigma_clip_fused_chunk_cuda_impl(
    const py::array_t<T, py::array::c_style | py::array::forcecast>& stack, const double rej_high,
    const double rej_low, const int max_iter, const py::object& mask_obj, const bool skip_zero_rgb,
    const ssize_t channels, Launcher launcher) {
...

    {
        ...
        launcher(stack.data(), mask_ptr, out_sum.mutable_data(), out_sq.mutable_data(),
                 out_n.mutable_data(), n_frames, plane_size, rej_high, rej_low, max_iter,
                 skip_zero_rgb, channels);
    }
    return py::make_tuple(out_sum, out_sq, out_n);
}

py::tuple sigma_clip_fused_chunk_cuda_dispatch(const py::array& stack, const double rej_high,
                                               const double rej_low, const int max_iter,
                                               const py::object& mask_obj, const bool skip_zero_rgb,
    ...
    if (py::isinstance<py::array_t<uint8_t>>(stack)) {
        ...
        return sigma_clip_fused_chunk_cuda_impl(stack_arr, rej_high, rej_low, max_iter, mask_obj,
                                                skip_zero_rgb, channels,
...
void bind_sigma_clip_fused_chunk_cuda_ops(py::module_& m) {
    m.def("sigma_clip_fused_chunk_cuda", &sigma_clip_fused_chunk_cuda_dispatch, py::arg("stack"),
