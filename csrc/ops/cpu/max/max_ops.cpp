...
template <typename T> inline T elementwise_max(T lhs, T rhs) {
...
template <typename T>
void max_combine_inplace_kernel(py::buffer_info& base_info, const py::buffer_info& fresh_info) {
...
template <typename T>
void threshold_max_merge_inplace_kernel(py::buffer_info& result_info,
                                        const py::buffer_info& frame_info,
                                        const py::buffer_info& mean_info,
                                        const py::buffer_info& std_info, const double n_sigma,
                                        const double weight) {
...
#pragma omp parallel for schedule(static)
    ...
    for (ssize_t i = 0; i < total; ++i) {
        ...
        const T candidate = frame_value > mean_ptr[i] + n_sigma_value * std_ptr[i]
...
template <typename T>
py::array_t<T>
max_combine_inplace_impl(hnw::MutableCArray<T> base,
...
py::array_t<T> threshold_max_merge_inplace_impl(
    ...
    const py::array_t<T, py::array::c_style | py::array::forcecast>& std_img, const double n_sigma,
    const double weight) {
    ...
    threshold_max_merge_inplace_kernel<T>(result_info, frame_info, mean_info, std_info, n_sigma,
                                          weight);
    return result;
}
...
py::array max_combine_dispatch(py::array base, const py::array& fresh) {
    ...
    if (py::isinstance<py::array_t<uint8_t>>(base)) {
        return max_combine_inplace_impl<uint8_t>(base.cast<hnw::MutableCArray<uint8_t>>(),
                                                 fresh.cast<py::array_t<uint8_t>>());
    }
    if (py::isinstance<py::array_t<uint16_t>>(base)) {
        return max_combine_inplace_impl<uint16_t>(base.cast<hnw::MutableCArray<uint16_t>>(),
                                                  fresh.cast<py::array_t<uint16_t>>());
    }
    if (py::isinstance<py::array_t<uint32_t>>(base)) {
        return max_combine_inplace_impl<uint32_t>(base.cast<hnw::MutableCArray<uint32_t>>(),
                                                  fresh.cast<py::array_t<uint32_t>>());
    }
    if (py::isinstance<py::array_t<uint64_t>>(base)) {
        return max_combine_inplace_impl<uint64_t>(base.cast<hnw::MutableCArray<uint64_t>>(),
                                                  fresh.cast<py::array_t<uint64_t>>());
    }
    if (py::isinstance<py::array_t<float>>(base)) {
        return max_combine_inplace_impl<float>(base.cast<hnw::MutableCArray<float>>(),
                                               fresh.cast<py::array_t<float>>());
    }
    if (py::isinstance<py::array_t<double>>(base)) {
        return max_combine_inplace_impl<double>(base.cast<hnw::MutableCArray<double>>(),
...
py::array threshold_max_merge_dispatch(py::array result, const py::array& frame,
                                       const py::array& mean_img, const py::array& std_img,
                                       const double n_sigma, py::object weight_obj) {
    ...
    if (py::isinstance<py::array_t<float>>(result)) {
        return threshold_max_merge_inplace_impl<float>(
            result.cast<hnw::MutableCArray<float>>(), frame.cast<py::array_t<float>>(),
            mean_img.cast<py::array_t<float>>(), std_img.cast<py::array_t<float>>(), n_sigma,
            weight);
    }
    if (py::isinstance<py::array_t<double>>(result)) {
        return threshold_max_merge_inplace_impl<double>(
            result.cast<hnw::MutableCArray<double>>(), frame.cast<py::array_t<double>>(),
            mean_img.cast<py::array_t<double>>(), std_img.cast<py::array_t<double>>(), n_sigma,
            weight);
    }
...
void bind_max_ops(py::module_& m) {
    ...
    m.def("threshold_max_merge", &threshold_max_merge_dispatch, py::arg("result"), py::arg("frame"),
          py::arg("mean_img"), py::arg("std_img"), py::arg("n_sigma"),
