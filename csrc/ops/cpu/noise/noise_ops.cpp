...
double select_quantile(std::vector<double>& values, const double q) {
    ...
    const double position =
        (static_cast<double>(values.size()) - 1.0) * clamped_q;
...
template <typename T>
void noise_fill_local_mean_kernel(
    py::buffer_info& out_info,
    const py::buffer_info& img_info,
    ...
    const int64_t channels =
        img_info.ndim == 3 ? static_cast<int64_t>(img_info.shape[2]) : 1;
...
#pragma omp parallel for schedule(static)
    ...
    for (int64_t c = 0; c < channels; ++c) {
        std::vector<double> sum_integral(
            static_cast<size_t>(integral_size), 0.0);
        std::vector<int32_t> count_integral(
            static_cast<size_t>(integral_size), 0);
        ...
        for (int64_t ey = 0; ey < ext_height; ++ey) {
            ...
            for (int64_t ex = 0; ex < ext_width; ++ex) {
                ...
                sum_integral[static_cast<size_t>(integral_idx)] =
                    sum_integral[static_cast<size_t>(previous_row + ex + 1)]
                    + row_sum;
                count_integral[static_cast<size_t>(integral_idx)] =
                    count_integral[static_cast<size_t>(previous_row + ex + 1)]
                    + row_count;
            }
        }
        ...
        for (int64_t y = 0; y < height; ++y) {
            ...
            for (int64_t x = 0; x < width; ++x) {
                ...
                const double sum =
                    sum_integral[static_cast<size_t>(bottom_right)]
                ...
                const int32_t count =
                    count_integral[static_cast<size_t>(bottom_right)]
                ...
                out_ptr[dst_idx] =
                    count > 0 ? static_cast<T>(sum / static_cast<double>(count))
                              : static_cast<T>(0);
            }
        }
    }
...
template <typename T>
py::array_t<T> noise_fill_local_mean_impl(
    ...
    auto out_info = out.request();
    noise_fill_local_mean_kernel<T>(
        out_info, img_info, mask_info, kernel_size);
    return out;
}

py::array noise_fill_local_mean_dispatch(
    const py::array& img,
    const py::array& mask,
    const int64_t kernel_size) {
    if (py::isinstance<py::array_t<float>>(img)) {
        return noise_fill_local_mean_impl<float>(
            img.cast<py::array_t<float>>(),
            mask.cast<py::array_t<bool>>(),
            kernel_size);
    }
    if (py::isinstance<py::array_t<double>>(img)) {
        return noise_fill_local_mean_impl<double>(
            img.cast<py::array_t<double>>(),
            mask.cast<py::array_t<bool>>(),
            kernel_size);
    }
...
py::object noise_equalization_params_impl(
    ...
    const bool n_matches_pixels =
        max_info.ndim == 3 && n_info.ndim == 2
...
#pragma omp critical
...

    {
        ...
        for (ssize_t i = 0; i < total; ++i) {
            ...
            ratio_values.push_back(
                (static_cast<double>(max_ptr[i]) - static_cast<double>(mean_ptr[i]))
                / sigma);
        }
    }
    ...
    const double sigma_ref =
        minus_only ? 0.0 : select_quantile(sigma_values, 0.5);
    ...
    for (ssize_t c = 0; c < channels; ++c) {
        ...
        const double variance =
            std::max(0.0,
                     channel_sq_sum[static_cast<size_t>(c)] / static_cast<double>(pixels)
                     - mean * mean);
...
#pragma omp parallel for schedule(static)
        ...
        for (ssize_t i = 0; i < total; ++i) {
            ...
            const double threshold_value =
                channel_mean[static_cast<size_t>(c)]
...
py::object noise_equalization_params_dispatch(
    const py::array& max_img,
    const py::array& mean_img,
    const py::array& std_img,
    const py::array& n_img,
    const double top_fraction,
    const double sigma_reject,
    const bool minus_only) {
...
void bind_noise_ops(py::module_& m) {
    ...
    m.def("noise_fill_local_mean",
          &noise_fill_local_mean_dispatch,
          py::arg("img"),
          py::arg("mask"),
          py::arg("kernel_size") = 21,
          "Fill masked pixels with a reflected-border local mean.");
    m.def("noise_equalization_params",
          &noise_equalization_params_dispatch,
          py::arg("max_img"),
          py::arg("mean_img"),
          py::arg("std_img"),
          py::arg("n_img"),
          py::arg("top_fraction") = 0.02,
          py::arg("sigma_reject") = 3.0,
          py::arg("minus_only") = false,
