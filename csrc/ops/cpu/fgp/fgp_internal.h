...
inline void validate_accumulator_shapes(const py::array& sum_mu, const py::array& square_sum,
...
inline void validate_masked_shapes(const py::array& sum_mu, const py::array& square_sum,
                                   const py::array& n, const py::array& fresh,
                                   const py::array& mask, const char* op_name) {
...
inline bool is_pixel_zero_rgb(const FreshT* HNW_RESTRICT ptr, ssize_t base, ssize_t channels) {
    for (ssize_t channel = 0; channel < std::min<ssize_t>(channels, 3); ++channel) {
