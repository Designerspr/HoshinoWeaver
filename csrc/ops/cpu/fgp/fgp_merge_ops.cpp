...
void fgp_add_inplace_kernel(py::buffer_info& sum_info, py::buffer_info& square_info,
                            py::buffer_info& count_info, const py::buffer_info& other_sum_info,
    ...
    const auto* HNW_RESTRICT other_sum_ptr = static_cast<const SumT*>(other_sum_info.ptr);
    const auto* HNW_RESTRICT other_square_ptr = static_cast<const SquareT*>(other_square_info.ptr);
    const auto* HNW_RESTRICT other_count_ptr = static_cast<const CountT*>(other_count_info.ptr);
...
#pragma omp parallel for schedule(static)
    ...
    for (ssize_t i = 0; i < total; ++i) {
        ...
        square_ptr[i] = static_cast<SquareT>(square_ptr[i] + other_square_ptr[i]);
...
void dispatch_add_count_dtype(hnw::MutableCArray<SumT> sum_mu,
                              hnw::MutableCArray<SquareT> square_sum, py::array n,
                              const py::array& other_sum_mu, const py::array& other_square_sum,
    ...
    if (py::isinstance<py::array_t<uint16_t>>(n)) {
        ...
        auto other_square =
            other_square_sum
                .cast<py::array_t<SquareT, py::array::c_style | py::array::forcecast>>();
...
void dispatch_add_square_dtype(hnw::MutableCArray<SumT> sum_mu, py::array square_sum, py::array n,
                               const py::array& other_sum_mu, const py::array& other_square_sum,
                               const py::array& other_n) {
    if (py::isinstance<py::array_t<uint32_t>>(square_sum)) {
        dispatch_add_count_dtype<SumT, uint32_t>(sum_mu,
                                                 square_sum.cast<hnw::MutableCArray<uint32_t>>(), n,
                                                 other_sum_mu, other_square_sum, other_n);
        return;
    }
    if (py::isinstance<py::array_t<uint64_t>>(square_sum)) {
        dispatch_add_count_dtype<SumT, uint64_t>(sum_mu,
                                                 square_sum.cast<hnw::MutableCArray<uint64_t>>(), n,
                                                 other_sum_mu, other_square_sum, other_n);
        return;
    }
    if (py::isinstance<py::array_t<double>>(square_sum)) {
        dispatch_add_count_dtype<SumT, double>(sum_mu,
                                               square_sum.cast<hnw::MutableCArray<double>>(), n,
                                               other_sum_mu, other_square_sum, other_n);
        return;
    }
...
void dispatch_add_sum_dtype(py::array sum_mu, py::array square_sum, py::array n,
                            const py::array& other_sum_mu, const py::array& other_square_sum,
                            const py::array& other_n) {
    if (py::isinstance<py::array_t<uint16_t>>(sum_mu)) {
        dispatch_add_square_dtype<uint16_t>(sum_mu.cast<hnw::MutableCArray<uint16_t>>(), square_sum,
                                            n, other_sum_mu, other_square_sum, other_n);
        return;
    }
    if (py::isinstance<py::array_t<uint32_t>>(sum_mu)) {
        dispatch_add_square_dtype<uint32_t>(sum_mu.cast<hnw::MutableCArray<uint32_t>>(), square_sum,
                                            n, other_sum_mu, other_square_sum, other_n);
        return;
    }
    if (py::isinstance<py::array_t<uint64_t>>(sum_mu)) {
        dispatch_add_square_dtype<uint64_t>(sum_mu.cast<hnw::MutableCArray<uint64_t>>(), square_sum,
                                            n, other_sum_mu, other_square_sum, other_n);
        return;
    }
    if (py::isinstance<py::array_t<double>>(sum_mu)) {
        dispatch_add_square_dtype<double>(sum_mu.cast<hnw::MutableCArray<double>>(), square_sum, n,
                                          other_sum_mu, other_square_sum, other_n);
        return;
    }
...
void validate_add_shapes(const py::array& sum_mu, const py::array& square_sum, const py::array& n,
                         const py::array& other_sum_mu, const py::array& other_square_sum,
                         const py::array& other_n) {
...
void fgp_add_dispatch(py::array sum_mu, py::array square_sum, py::array n,
                      const py::array& other_sum_mu, const py::array& other_square_sum,
                      const py::array& other_n) {
    validate_add_shapes(sum_mu, square_sum, n, other_sum_mu, other_square_sum, other_n);
    dispatch_add_sum_dtype(sum_mu, square_sum, n, other_sum_mu, other_square_sum, other_n);
}
...
void bind_fgp_merge_ops(py::module_& m) {
    m.def("fgp_add", &fgp_add_dispatch, py::arg("sum_mu"), py::arg("square_sum"), py::arg("n"),
          py::arg("other_sum_mu"), py::arg("other_square_sum"), py::arg("other_n"),
