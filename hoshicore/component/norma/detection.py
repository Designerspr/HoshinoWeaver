...
try:
    ...
    logger.warning(
        "Custom-op median_filter_2d not available; falling back to slower "
        "Python implementation")
    median_filter_2d = None
...
def _bandpass_dog(img_blr: np.ndarray,
                  resize_factor: float = 0.25) -> np.ndarray:
    h, w = img_blr.shape
...
def detect_starmask_by_threshold_with_response(
    ...
    if med_algo == "median":
        if ksize <= 5 or median_filter_2d is None:
...
def detect_star_points(
    ...
    if img_gray.dtype != np.float64:
        if np.issubdtype(img_gray.dtype, np.integer):
            img_gray = img_gray.astype(np.float64) / np.iinfo(
                img_gray.dtype).max
        else:
    ...
    if mask is None:
        ...
        logger.debug(
            "Mask calculation Complete (auto mask disabled; using full image)")
    else:
        mask = mask > 0
    ...
    valid_stars = np.logical_and(areas > MIN_STAR_AREA, eccentricities < 0.85)
    valid_stars = np.logical_and(
        np.logical_and(valid_stars, areas > area_percentile), intensities
    ...
    return DetectedStars(positions=star_pts,
                         volumes=areas * intensities,
