...
from hoshicore.component.norma.alignment import optimize_alignment, warp_image_by_remap
...
def _run_alignment_pipeline_once(
    ...
    *,
    ...
    ref_geo = _time_stage(
        stage_times,
        "reference_geometry",
        lambda: _make_geometry_eager(reference),
    )

    ref_camera = None
    if method == "camera_model":
    ...
    for frame in frames[1:]:
        try:
            src_geo = _time_stage(
                stage_times,
                "source_geometry",
                lambda: _make_geometry_eager(frame),
            )
            _time_stage(stage_times, "star_count_check", lambda: _check_star_count(ref_geo, src_geo))
            match = _time_stage(stage_times, "match", lambda: _match_stars(ref_geo, src_geo))

            if method == "homography":
                def warp_homography() -> np.ndarray:
                    h_src_to_ref = np.linalg.inv(match.init_homography)
                ...
                aligned = _time_stage(
                    stage_times,
                    "remap",
                    lambda: warp_image_by_remap(
                        frame,
                        optimized.camera2_refined,
                        optimized.camera1_refined,
                        output_size,
                    ),
                )
