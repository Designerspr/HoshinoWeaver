...
def set_default_logger(debug_mode, work_mode):
    ...
    global trail_logger
    if not trail_logger.is_stopped:
        trail_logger.log(LV_ERROR, "Can not set a running logger.")
        return -1
    if debug_mode:
        trail_logger.log_level=LV_DROPPED
    elif work_mode=="backend":
        trail_logger.flush=True
        trail_logger.log_level=LV_DEBUG
    else:
        trail_logger.log_level=LV_INFO
